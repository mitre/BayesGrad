...
#SBATCH -o ./logs/test_loc/%u-%x-job%j.out
#SBATCH --ntasks=1
...
"""
    This script runs model training 20 times for the localization control model
    so that we can account for differences in individual trained models in our results.
"""
...
singularity exec --nv --bind /q/PET-MBF:/q/PET-MBF ../../Singularity/xnn4rad.sif python3 ../train_VGGDrop_norm_abn.py \
    ...
    --problem=abn_localization \
    --suffix="loc${SLURM_ARRAY_TASK_ID}" \
    --stress \
    --rest
