...
This file contains functions related to loading the data. This is essentially
the same as data_utils.py in PET aim 1. This analysis assumes that data storage
and directory structyre is set up in the same as it was in PET aim 1. 
...
def load_dataset(data_dir, dataset, problem, split, val_col=None):
    ...
    number of data instances. C[0] is stress, C[1] is rest, C[2] is reserve
    C[3] is difference.
    ...
    if problem == 'norm_abn':
        ...
        raise NotImplementedError("problem must be 'norm_abn'")
...
def format_col_names(pet):
    """
    # small dataset has 1 avg value for each vessel territory for each measure
    # (rest, stress, reserve, difference)
...
def load_data(data_path):
    """
    Load and format pet scan dataset; exclude samples as outlined in description
    of exclude(); Works for both small and full 17 segment data
    ...
    -------
    pet : pd.DataFrame
        Formatted DataFrame with pet scan
    """
    pet = pd.read_excel(data_path, header=[0,1], engine='openpyxl')
    pet = format_col_names(pet)

    pet = flatten_multiindex(pet)

    # remove excluded scans
    pet = exclude(pet)

    # For now, simplify scar classes to binary
    pet.loc[pet['scar_lad'] != 0, 'scar_lad'] = 1
    pet.loc[pet['scar_lcx'] != 0, 'scar_lcx'] = 1
    pet.loc[pet['scar_rca'] != 0, 'scar_rca'] = 1
